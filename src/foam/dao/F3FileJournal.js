  ...
  methods: [
    {
      ...
      documentation: `The AssemblyLine a replay parses and applies through.
        ...
        the MDAO bulk-loads it) the line is sharded: every worker parses, and
        applies the entries whose id hashes to its shard, so one id is always
        applied by one thread in journal order and nothing else is serial.
      ...
      javaCode: `
        ...
        if ( dao instanceof foam.dao.BulkLoadDAO )
          return new foam.util.concurrent.BatchingAssemblyLine(new foam.util.concurrent.OwnedShardAssemblyLine(x, "replay", ((foam.dao.BulkLoadDAO) dao).shards()));
