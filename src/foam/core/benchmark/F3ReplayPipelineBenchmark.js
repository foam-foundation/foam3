...
foam.CLASS({
  ...
  documentation: `Replay wall and CPU time per apply-stage shape, on the
    ...
    - line: simple (parallel parse, one apply thread, the line before this
      change), owned (every worker parses and applies its own id shard, the
  ...
  javaImports: [
    ...
    'foam.util.concurrent.OwnedShardAssemblyLine',
  ...
  properties: [
    ...
    },
    {
      class: 'String',
      name: 'lines',
      documentation: 'floor, simple[-P], owned[-P]; P threads, default cores - 1.',
      value: 'simple,owned'
    },
    {
      class: 'String',
  ...
  javaCode: `
    ...
    static class VariantF3FileJournal extends F3FileJournal {
      ...
      public AssemblyLine createReplayLine(X x, DAO dao) {
        if ( line_.startsWith("owned") ) return new BatchingAssemblyLine(new OwnedShardAssemblyLine(x, "replay", threadsOf(line_)));
      ...
      javaCode: `
        ...
        for ( int i = 0 ; i < cells ; i++ ) {
          ...
          if ( floor ) {
            ...
            // One shard per apply worker; the single apply thread gets one map.
            dao = new BulkLoadDAO(getX(), of, lineOf(cell).startsWith("owned") ? threadsOf(lineOf(cell)) : 1);
          }
