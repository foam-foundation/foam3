  ...
  properties: [
    {
      ...
      value: function(alt, seq, seq0, seq1, repeat, repeat0, optional, literal, literalIC, str, substring, sug, sym, range) {
        ...
        this.of.getAxiomsByClass(foam.lang.Constant).forEach(function(c) {
          fields.push(sug(literalIC(c.name, c.value), {text: c.name, category: 'constant'}));
        });
...
//          EXPR: sym('concat'),
          ...
          // A comparison between two columns. It has to come before
          ...
          // text "b" instead of against column b - and reports no error.
          //
          // Only matches when the right side is itself a column name, so
